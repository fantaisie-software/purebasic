...
; Ascii only MemoryStringCompare to have much faster compare as we don't need unicode support for token parsing (https://www.purebasic.fr/english/viewtopic.php?t=78887)
;
Global  Dim NoCaseAsciiLookup(255)
For k=0 To 255
  NoCaseAsciiLookup(k) = k
Next

; Map A-Z to a-z, so we have easy case compare lookup
;
For k='A' To 'Z'
  NoCaseAsciiLookup(k) = 'a'+(k-'A')
Next

Procedure CompareMemoryStringFast(*String1.Character, *String2.Character, Mode, Length, Flags)
  
  ; Fast mode is only for Unicode with CaseSensitive compare
  ;
  If Mode <> #PB_String_NoCase Or Flags <> #PB_Unicode
    ProcedureReturn CompareMemoryString(*String1, *String2, Mode, Length, Flags)
  EndIf
  ...
  ; Handle null case for both strings
  ;
  If *String1 = #Null
    If *String2 = #Null
      ProcedureReturn #PB_String_Equal
  ...
  While *String1\c
    c1.c = NoCaseAsciiLookup(*String1\c & $FF)
    c2.c = NoCaseAsciiLookup(*String2\c & $FF)
    
    If c1 = c2 ; Same char, go to the next
      *String1+SizeOf(Character)
      *String2+SizeOf(Character)
      
      Length-1
      If Length = 0 ; Job finished !
        ProcedureReturn #PB_String_Equal
      EndIf
  ...
  If *String2\c = 0 ; Also end of String2, so it's equal
    ProcedureReturn #PB_String_Equal
  EndIf
  
  ProcedureReturn #PB_String_Lower ; Different becasuse size is different
EndProcedure

Macro CompareMemoryString(String1, String2, Mode=#PB_String_CaseSensitive, Length=-1, Flags=#PB_Unicode)
  CompareMemoryStringFast(String1, String2, Mode, Length, Flags)
EndMacro
