...
Procedure AutoIndent()
  ...
  SendEditorMessage(#SCI_BEGINUNDOACTION, 0, 0)
  Changed = UpdateIndent(LineStart-1, LineEnd-1)
  SendEditorMessage(#SCI_ENDUNDOACTION, 0, 0)
  
  ; This is required to update the keyword underline data
  ;
  If Changed And PartialSourceScan(*ActiveSource, LineStart-1, LineEnd-1)
    UpdateFolding(*ActiveSource, LineStart-1, LineEnd-1)
    UpdateProcedureList()
    UpdateVariableViewer()
  EndIf
...
Procedure FormatSourceIndentation()
  If *ActiveSource\IsCode = 0
    ProcedureReturn
  EndIf

  Anchor = SendEditorMessage(#SCI_GETANCHOR, 0, 0)
  Current = SendEditorMessage(#SCI_GETCURRENTPOS, 0, 0)
  AnchorLine = SendEditorMessage(#SCI_LINEFROMPOSITION, Anchor, 0)
  CurrentLine = SendEditorMessage(#SCI_LINEFROMPOSITION, Current, 0)
  AnchorColumn = CountCharacters(*ActiveSource\EditorGadget, SendEditorMessage(#SCI_POSITIONFROMLINE, AnchorLine, 0), Anchor)
  CurrentColumn = CountCharacters(*ActiveSource\EditorGadget, SendEditorMessage(#SCI_POSITIONFROMLINE, CurrentLine, 0), Current)
  LastLine = SendEditorMessage(#SCI_GETLINECOUNT, 0, 0) - 1

  If UpdateIndent(0, LastLine)
    If PartialSourceScan(*ActiveSource, 0, LastLine)
      UpdateFolding(*ActiveSource, 0, LastLine)
      UpdateProcedureList()
      UpdateVariableViewer()
    EndIf

    ; Restore character columns without flushing events during Save All or Auto-save.
    AnchorColumn = Min(AnchorColumn, Len(GetLine(AnchorLine)))
    CurrentColumn = Min(CurrentColumn, Len(GetLine(CurrentLine)))
    Anchor = SendEditorMessage(#SCI_POSITIONRELATIVE, SendEditorMessage(#SCI_POSITIONFROMLINE, AnchorLine, 0), AnchorColumn)
    Current = SendEditorMessage(#SCI_POSITIONRELATIVE, SendEditorMessage(#SCI_POSITIONFROMLINE, CurrentLine, 0), CurrentColumn)
    SendEditorMessage(#SCI_SETSEL, Anchor, Current)
  EndIf
EndProcedure
